...
async def call_model_with_messages(state: ThreadState, config: RunnableConfig) -> dict:
    ...
    try:
        # Attempt to use with_structured_output
        ai_message = model.with_structured_output(Strategy).invoke(system_prompt)
    except NotImplementedError:
        logger.warning(f"Model {model.__class__.__name__} does not support with_structured_output. Falling back to parser.")
        ai_message = (model | parser).invoke(system_prompt)
    except Exception as e:
        logger.error(f"Error invoking model with structured output: {e}. Falling back to parser.")
        ai_message = (model | parser).invoke(system_prompt)
