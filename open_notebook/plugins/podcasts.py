...
class PodcastConfig(ObjectModel):
    ...
    dialogue_structure: str = Field(default="")
    ...
    @field_validator("dialogue_structure", mode="before")
    @classmethod
    def convert_list_to_str(cls, value):
        if isinstance(value, list):
            if not value: # Empty list
                return "" # Convert empty list to empty string
            # For non-empty list, convert to string representation or join
            # Using str() for now to capture all items if multiple existed unexpectedly
            return str(value) 
        return value
